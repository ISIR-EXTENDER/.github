...
Kept readable for the record and for rollback; not a starting point for new work.

| Repository | Status |
| --- | --- |
| [`extender_ui`](https://github.com/ISIR-EXTENDER/extender_ui) · [`tablet_interface`](https://github.com/ISIR-EXTENDER/input_interfaces/tree/main/tablet_interface) | The previous tablet app and its backend, retired in September 2026 (tags `v1.0.0` and `tablet_interface/v1.0.0`). |
| [`sandbox_controller`](https://github.com/ISIR-EXTENDER/sandbox_controller) · [`controllers`](https://github.com/ISIR-EXTENDER/controllers) · [`robot_interfaces`](https://github.com/ISIR-EXTENDER/robot_interfaces) | The previous controller layer on the `/teleop_cmd` contract, including the Franka FR3 support that has not been ported. |
| [`ISIR-EXTENDER/explorer_stack`](https://github.com/ISIR-EXTENDER/explorer_stack) | Humble-era fork; the workspace now imports Orthopus `develop` directly. |
...
Ubuntu 24.04 and ROS 2 Jazzy are the baseline.
...
Then launch one of the robots above, and drive it from a joystick, or from the
tablet interface by following
[Bloom's getting-started tutorial](https://github.com/ISIR-EXTENDER/bloom/blob/main/docs/tutorials/getting-started.md).
System packages, Kinova on Jazzy, the Humble upgrade path and the known Explorer
simulation workarounds are in the
[workspace README](https://github.com/ISIR-EXTENDER/extender_workspace#readme).
